describe Colossus::Engine::MemoryMonitor do
  ...
  describe '#gc_ttl' do
    ...
    it 'keeps unexpired session' do
      ...
      subject.gc_ttl
      expect(subject.client_sessions.length).to eq(1)
    end
    ...
    it 'removes expired session in sessions stores' do
      ...
      subject.gc_ttl
      expect(subject.client_sessions.values.first.sessions.length).to eq(1)
    end
  end
