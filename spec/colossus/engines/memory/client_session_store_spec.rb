describe Colossus::Engine::Memory::ClientSessionStore do
  let(:client_session_store) { subject }

  describe '#sessions' do
    it 'is a Hash' do
      expect(client_session_store.sessions).to an_instance_of(Hash)
    end

    it 'keeps sessions' do
      client_session_store['session_id'] = 'active'
      client_session_store['session_id_bis'] = 'active'
      expect(client_session_store.sessions.count).to eq(2)
    end
  end
